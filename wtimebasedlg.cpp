...
#include "CommClass.h"
WTimeBaseDlg::WTimeBaseDlg(QWidget *parent) :
    QDialog(parent)
{
    initDlg();
...
void WTimeBaseDlg::on_changeSpecialModel()
{
    clearInterface();
...
void WTimeBaseDlg::on_dataChange()
...
void WTimeBaseDlg::on_tabChange(int index)
{
    clearInterface();
...
void WTimeBaseDlg::on_clickTimebaseIndex(QModelIndex index)
{
    if(!index.isValid())
    {
        setCheckBoxEnable(false);
...
void WTimeBaseDlg::on_clickSchIndex(QModelIndex index)
{
...
void WTimeBaseDlg::showSumSelect()
{
    //m_bFlagBtn = true;
...
void WTimeBaseDlg::setDataProcess(PDataProcess *dp)
{
    m_pDataProcess = dp;
...
void WTimeBaseDlg::on_appBtn()
{
    m_pDataProcess->sendTimeBaseToLib(m_bClusterClick);
}

void WTimeBaseDlg::on_readBtn()
{
...
void WTimeBaseDlg::on_addEventBtn()
{
...
void WTimeBaseDlg::on_delEventBtn()
{
    QStandardItemModel *model = m_pDataProcess->m_pModelEvent;
...
void WTimeBaseDlg::on_addSchBtn()
{
...
void WTimeBaseDlg::on_editSchBtn()
{
...
void WTimeBaseDlg::on_delSchBtn()
{
...
void WTimeBaseDlg::on_appSchBtn()
{
...
void WTimeBaseDlg::on_readSchBtn()
{
...
void WTimeBaseDlg::on_rcvTimeBaseResponse(char *para1, int)
{
    if(*para1==0x01)
    {
        QString str;
...
void WTimeBaseDlg::on_rcvScheduleResponse(char *para1, int)
{
    if(*para1==0x01)
    {
        QString str;
...
void WTimeBaseDlg::on_tbViewSelect(QModelIndex index)
{
    if(index.isValid())
...
/*
*/
void WTimeBaseDlg::on_schViewSelect(QModelIndex index)
{
    if(index.isValid())
    {
...
void WTimeBaseDlg::on_eventViewSelect(QModelIndex index)
{
    if(index.isValid())
    {
...
void WTimeBaseDlg::on_updatePatternDeligate()
{
    setPatternDeligate();
}

void WTimeBaseDlg::on_clusterClick(bool b)
{
    m_bClusterClick = b;
    ctBtn->setVisible(b);
...
void WTimeBaseDlg::on_ctBtn()
{
//    m_pDataProcess->sendClusterPlan();
}

void WTimeBaseDlg::on_cbAllDay(bool b)
{
    foreach(QCheckBox* cb,monthDayList)
    {
        cb->setChecked(b);
...
void WTimeBaseDlg::setInterfaceFromData(TimeBase_Table *tbTimeBase)
{
...
//    qDebug()<<tr("quint32 lMonthDay %1,qint32 lMonthDay%2").arg(lMonthDay).arg((qint32)lMonthDay);
    ...
    if(bNo>byte(SUM_TIMEBASE_INDEX))//夏日制
    {
//        tabWidget->setCurrentIndex(0);
        bool bFlag = false;
        for(int i=0;i<11;i++)
...
void WTimeBaseDlg::getDataFromInterface(TimeBase_Table *tbTimeBase)
{
    quint32 lMonthDay = 0;
    quint16 uMonth = 0;
    quint8 bWeekDay = 0;
...
void WTimeBaseDlg::setPatternDeligate()
{
    if(m_pDataProcess!=NULL)
    {
        if(patternDeligate!=NULL)
...
void WTimeBaseDlg::timerEvent(QTimerEvent *e)
{
    m_bTimeOut = true;
...
void WTimeBaseDlg::startTimeOut()
{
    if(!m_bTimer)
    {
...
void WTimeBaseDlg::clearInterface()
{
...
void WTimeBaseDlg::setCheckBoxEnable(bool b)
{
...
quint8 WTimeBaseDlg::getBaseNum(QString str)
{
    quint8 num = 0;
    if ( 0 == str.left(6).compare(tr("夏日制时基表")) )
    {
        qDebug()<<"WTimeBaseDlg::getBaseNum"<<SUMMERYDAY;
    ...
    else if( 0 == str.left(6).compare(tr("冬日制时基表")))
    {
        qDebug()<<"WTimeBaseDlg::getBaseNum"<<WINNERDAY;
    ...
    else if( 0 == str.left(6).compare(tr("特殊日时基表")))
    {
        qDebug()<<"WTimeBaseDlg::getBaseNum"<<SPECIALDAY;
        num = SPECIAL_TIMEBASE_INDEX;
        tabWidget->setCurrentIndex(1);

    }
    qDebug()<<"WTimeBaseDlg::getBaseNum"<<str<<"base"<<num<<str.left(6).compare(tr("夏日制时基表"))<<str.left(6).compare(tr("冬日制时基表"))<<str.left(6).compare(tr("特殊日时基表"));
    return num;
}

//void WTimeBaseDlg::showByIndex()
//{

//    if(m_ucTimeBaseIndex>(WIN_TIMEBASE_INDEX-1))
//    {
//        tabWidget->setCurrentIndex(0);
...
//        winRBtn->click();
//        winRBtn->setChecked(true);
//        winRBtn->setVisible(true);
//        winRBtn->setEnabled(true);
//        sumRBtn->setVisible(false);
//        sumBtn->setVisible(false);

//    }
//    else if(m_ucTimeBaseIndex>(SUM_TIMEBASE_INDEX-1))
//    {
//        tabWidget->setCurrentIndex(0);
//        tabWidget->setTabEnabled(0,true);
//        tabWidget->setTabEnabled(1,false);
//        sumRBtn->click();
//        sumRBtn->setChecked(true);
//        sumRBtn->setVisible(true);
//        sumRBtn->setEnabled(true);
//        sumBtn->setVisible(true);
//        sumBtn->setEnabled(true);
//        winRBtn->setVisible(false);
//    }
//    else
//    {
//        tabWidget->setCurrentIndex(1);
